...
from src.database.models import User, Category, Movie, Episode, Admin, Channel
...
class Collection:
    ...
    async def find(self, f: Optional[dict] = None, count: int = 100000, offset: int = 0) -> List:
        ...
        """
        cursor = self.collection.find(f or {}).skip(offset).limit(count)
        data = await cursor.to_list(length=count)
        list_models = []
        for item in data:
            # Mongo ObjectId ni oddiy stringga o‘girib qo‘yamiz
